...
// +kubebuilder:rbac:groups=library.grafana.kubeform.com,resources=panels,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=library.grafana.kubeform.com,resources=panels/status,verbs=get;update;patch

func (r *PanelReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("panel", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Panel")
...
func (r *PanelReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &libraryv1alpha1.Panel{}); err != nil {
			klog.Error(err, "unable to set up auditor", libraryv1alpha1.Panel{}.APIVersion, libraryv1alpha1.Panel{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&libraryv1alpha1.Panel{}).
