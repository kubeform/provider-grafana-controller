...
// LearningJobReconciler reconciles a LearningJob object
type LearningJobReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=machine.grafana.kubeform.com,resources=learningjobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=machine.grafana.kubeform.com,resources=learningjobs/status,verbs=get;update;patch

func (r *LearningJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("learningjob", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LearningJob")
...
func (r *LearningJobReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &machinev1alpha1.LearningJob{}); err != nil {
			klog.Error(err, "unable to set up auditor", machinev1alpha1.LearningJob{}.APIVersion, machinev1alpha1.LearningJob{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&machinev1alpha1.LearningJob{}).
