...
// MonitoringProbeReconciler reconciles a MonitoringProbe object
type MonitoringProbeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synthetic.grafana.kubeform.com,resources=monitoringprobes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synthetic.grafana.kubeform.com,resources=monitoringprobes/status,verbs=get;update;patch

func (r *MonitoringProbeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("monitoringprobe", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MonitoringProbe")
...
func (r *MonitoringProbeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &syntheticv1alpha1.MonitoringProbe{}); err != nil {
			klog.Error(err, "unable to set up auditor", syntheticv1alpha1.MonitoringProbe{}.APIVersion, syntheticv1alpha1.MonitoringProbe{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&syntheticv1alpha1.MonitoringProbe{}).
...
func (r *MonitoringProbeReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for MonitoringProbe")
			return nil
		}
		...
		resourceList := &syntheticv1alpha1.MonitoringProbeList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list MonitoringProbe")
			return nil
		}
