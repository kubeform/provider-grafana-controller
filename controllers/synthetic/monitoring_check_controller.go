...
// MonitoringCheckReconciler reconciles a MonitoringCheck object
type MonitoringCheckReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synthetic.grafana.kubeform.com,resources=monitoringchecks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synthetic.grafana.kubeform.com,resources=monitoringchecks/status,verbs=get;update;patch

func (r *MonitoringCheckReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("monitoringcheck", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MonitoringCheck")
...
func (r *MonitoringCheckReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &syntheticv1alpha1.MonitoringCheck{}); err != nil {
			klog.Error(err, "unable to set up auditor", syntheticv1alpha1.MonitoringCheck{}.APIVersion, syntheticv1alpha1.MonitoringCheck{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&syntheticv1alpha1.MonitoringCheck{}).
