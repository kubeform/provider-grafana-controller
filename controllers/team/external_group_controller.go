...
// ExternalGroupReconciler reconciles a ExternalGroup object
type ExternalGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=team.grafana.kubeform.com,resources=externalgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=team.grafana.kubeform.com,resources=externalgroups/status,verbs=get;update;patch

func (r *ExternalGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("externalgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ExternalGroup")
...
func (r *ExternalGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &teamv1alpha1.ExternalGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", teamv1alpha1.ExternalGroup{}.APIVersion, teamv1alpha1.ExternalGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&teamv1alpha1.ExternalGroup{}).
