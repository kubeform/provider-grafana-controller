...
// PreferencesReconciler reconciles a Preferences object
...
// +kubebuilder:rbac:groups=team.grafana.kubeform.com,resources=preferences,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=team.grafana.kubeform.com,resources=preferences/status,verbs=get;update;patch

func (r *PreferencesReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("preferences", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Preferences")
...
func (r *PreferencesReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &teamv1alpha1.Preferences{}); err != nil {
			klog.Error(err, "unable to set up auditor", teamv1alpha1.Preferences{}.APIVersion, teamv1alpha1.Preferences{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&teamv1alpha1.Preferences{}).
