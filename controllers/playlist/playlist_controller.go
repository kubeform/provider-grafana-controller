...
// PlaylistReconciler reconciles a Playlist object
...
// +kubebuilder:rbac:groups=playlist.grafana.kubeform.com,resources=playlists,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=playlist.grafana.kubeform.com,resources=playlists/status,verbs=get;update;patch

func (r *PlaylistReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("playlist", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Playlist")
...
func (r *PlaylistReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &playlistv1alpha1.Playlist{}); err != nil {
			klog.Error(err, "unable to set up auditor", playlistv1alpha1.Playlist{}.APIVersion, playlistv1alpha1.Playlist{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&playlistv1alpha1.Playlist{}).
