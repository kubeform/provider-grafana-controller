...
// SourcePermissionReconciler reconciles a SourcePermission object
type SourcePermissionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.grafana.kubeform.com,resources=sourcepermissions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.grafana.kubeform.com,resources=sourcepermissions/status,verbs=get;update;patch

func (r *SourcePermissionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sourcepermission", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SourcePermission")
...
func (r *SourcePermissionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.SourcePermission{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.SourcePermission{}.APIVersion, datav1alpha1.SourcePermission{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.SourcePermission{}).
